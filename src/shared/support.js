import {
 	 	 	   ...
 	 	 	   SetTrainingSingle, SetPerformanceAppraisalSingle, SetOrganizationSingle, SetEmployeePersonalInfoSingle, SetEmlpoyeeProfileSingle
} from "./services";
...
fn.AddOrUpdateTraining = async (input, enums, excludesItems) => {
    return new Promise(async (resolve) => {
        ...
        global.Busy(true);
        let rslt = await SetTrainingSingle(data);
        global.Busy(false);
        if (rslt.status) {
            id = rslt.id;
            status = true;
...
fn.AddOrUpdatePerformanceAppraisal = async (input, excludesItems) => {
    return new Promise(async (resolve) => {
        let data = {}, status = false, id = null;
        let excludes = excludesItems || [];
        Object.values(input)
        ...
        global.Busy(true);
        let rslt = await SetPerformanceAppraisalSingle(data);
        global.Busy(false);
        if (rslt.status) {
            id = rslt.id;
            status = true;
...
fn.AddOrUpdateEmployeePersonalInfo = async (input, excludesItems) => {
    return new Promise(async (resolve) => {
        let data = {}, status = false, id = null;
        let excludes = excludesItems || [];
        Object.values(input)
