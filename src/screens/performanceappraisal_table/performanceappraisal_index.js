...
const columns = [
    { headerName: "Quarter", field: "Quarter", flex: 1 },
    { headerName: "Date", field: "Date", flex: 1 },
    { headerName: "EmployeeNotes", field: "EmployeeNotes", flex: 1 },
    { headerName: "ManagerComments", field: "ManagerComments", flex: 1 },
    { headerName: "HikePercentage", field: "HikePercentage", flex: 1 },
    { headerName: "AppraisalRating", field: "AppraisalRating", flex: 1 },
];
...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetPerformanceAppraisalsCount(query)
            .then(async (res) => {
                if (res.status) {
        ...
        await Api.GetPerformanceAppraisalsMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetPerformanceAppraisalSingle({ AppraisalID: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/PerformanceAppraisals/edit/${id}`;
        if (type === 'view') _route = `/PerformanceAppraisals/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                <Box style={{ width: '100%', paddingBottom: 5 }}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                                ...
                                onClick={() => NavigateTo("/PerformanceAppraisals/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'AppraisalID'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
