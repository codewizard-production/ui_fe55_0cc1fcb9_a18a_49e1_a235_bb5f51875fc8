import PerformanceAppraisalJsonConfig from "config/performanceappraisal_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "performanceappraisal", expand: "", exclude: [], func: Support.AddOrUpdatePerformanceAppraisal }
]

const FetchPerformanceAppraisalInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(PerformanceAppraisalJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of PerformanceAppraisalJsonConfig[elm]) {
...
const FetchPerformanceAppraisalDetails = async (performanceappraisalId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (performanceappraisalId) {
            global.Busy(true);
            let rslt
            // Get PerformanceAppraisal Details
            let $expand = [];
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetPerformanceAppraisalSingle(performanceappraisalId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetPerformanceAppraisalSingle(performanceappraisalId);
            }
            
            if (rslt.status) {

                const performanceappraisal = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? performanceappraisal : performanceappraisal[source];
...
const Extract = async (performanceappraisalId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchPerformanceAppraisalInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(performanceappraisalId)) {
                    await FetchPerformanceAppraisalDetails(performanceappraisalId, enums).then(({ row, backRow }) => {
