...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, performanceAppraisalId, numfields;
        const mapItems = MapItems;

        let performanceappraisal = row['performanceappraisal'];

        numfields = Helper.GetAllNumberFields(performanceappraisal);
        if (numfields.length > 0) Helper.UpdateNumberFields(performanceappraisal, numfields);


        // Add Or Update PerformanceAppraisal
        rslt = await Support.AddOrUpdatePerformanceAppraisal(performanceappraisal, []);
        if (rslt.status) {
            performanceAppraisalId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the PerformanceAppraisal and navigation entity if it is deos not exist
            let navItem = performanceappraisal.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "AppraisalID", value: parseInt(performanceAppraisalId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdatePerformanceAppraisal(data);
        ...
        global.AlertPopup("success", "PerformanceAppraisal is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
