...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], performanceAppraisalId, performanceappraisal, numfields;

        performanceappraisal = row['performanceappraisal'];
        performanceAppraisalId = row['performanceappraisal'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) performanceAppraisalId = rslt.id;
                            ...
                            rslt = await Support.AddOrUpdatePerformanceAppraisal(data, dropDownOptions);
        ...
        global.AlertPopup("success", "PerformanceAppraisal is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
